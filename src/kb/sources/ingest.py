...
async def ingest_source(
    *, api_base: str, domain: str, source: Source, project: str = "default"
) -> list[dict]:
    ...
    async with httpx.AsyncClient(timeout=180) as client:
        async for d in source.fetch():
            try:
                r = await client.post(
                    f"{api_base}/files",
                    data={"project": project, "domain": domain},
        ...
        # Kick the worker
        try:
            await client.post(f"{api_base}/ingest/run", json={"project": project, "domain": domain})
        except Exception:
