...
| `sibling-rag-service-retired` fails | The old `../rag-service` folder still exists, has deployable surfaces, or a fleet app still references it | Run `pnpm run audit:sibling-rag-service -- --json`, close OCR parity first, then delete the sibling folder when all blockers are clear. |
